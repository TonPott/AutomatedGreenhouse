...
void Ad5263SafeController::begin(TwoWire& wire) {
  ...
  digitalWrite(PIN_LIGHT_DIM_SHDN, LOW);
  relayOpen_ = true;
  shdnReleased_ = false;
  clearFault();
}

bool Ad5263SafeController::initializeSafeTarget() {
  enforceRelayOpen();
  assertShutdown();
...
bool Ad5263SafeController::applyBrightnessWhileOn(uint8_t percent) {
  const uint8_t clamped = clampPercent(percent);
  if (clamped == 0 || relayOpen_ || !shdnReleased_ || fault_ || !present_) {
...
bool Ad5263SafeController::releaseShutdown() {
  enforceRelayOpen();
  if (fault_ || !present_) {
    assertShutdown();
    return false;
...
bool Ad5263SafeController::closeRelay() {
  ...
  relayOpen_ = digitalRead(PIN_LIGHT_POWER) == LOW;
  if (relayOpen_) {
    setFault(kAd5263WriteFailed);
    return false;
  }
...
void Ad5263SafeController::openRelay() {
  enforceRelayOpen();
}

void Ad5263SafeController::assertShutdown() {
  enforceRelayOpen();
  if (shdnReleased_ || digitalRead(PIN_LIGHT_DIM_SHDN) != LOW) {
    digitalWrite(PIN_LIGHT_DIM_SHDN, LOW);
    delay(LIGHT_DIM_BOOT_SETTLE_MS);
  }
  shdnReleased_ = false;
...
void Ad5263SafeController::enforceRelayOpen() {
  digitalWrite(PIN_LIGHT_POWER, LOW);
  relayOpen_ = digitalRead(PIN_LIGHT_POWER) == LOW;
}
...
bool Ad5263SafeController::isRelayOpen() const {
  return relayOpen_;
}
