...
public:
  ...
  uint32_t getTotalTachPulses() const;
  uint32_t getLastWindowTachPulses() const;
  bool isTachInterruptAttached() const;
  int getTachInputLevel() const;
...
private:
  ...
  volatile uint32_t totalTachPulseCount_ = 0;

  uint32_t lastWindowTachPulseCount_ = 0;
  bool tachInterruptAttached_ = false;
