...
constexpr uint8_t SHT_PRIMARY_ADDRESS = SHT31_I2C_ADDRESS;
...
bool readLimit(uint8_t lsb, LimitValue& limit) {
  Wire.beginTransmission(SHT_PRIMARY_ADDRESS);
  ...
  if (Wire.requestFrom(static_cast<uint8_t>(SHT_PRIMARY_ADDRESS), static_cast<uint8_t>(3)) != 3) {
...
void initializeSht() {
  ...
  shtState.primaryPresent = probeI2cAddress(SHT_PRIMARY_ADDRESS);

  shtSensor.begin(Wire, SHT_PRIMARY_ADDRESS);
